...
### Todo list (Phase-3 wiring)

| Channel | Direction | Pattern | Renderer Signature | Payload |
|---|---|---|---|---|
| `capture:list` | renderer -> main | invoke | `listTodos()` | -> `TodoItem[]` |

`capture:list` is the read channel that makes the captured todos reachable in the running app: App.tsx reads it so HomeView can render the M15 triage panel, the M18 parking / resurfacing / morning-ritual surfaces, and the Tier-5 `@now` todos. Without it the renderer could append, count, and mutate-by-id, but never LIST the items to display them. The main handler returns `readTodosFile(captureDir).items` (which degrades a corrupt store to an empty array); the items carry only the structured `TodoItem` fields, and the text is DISPLAY-ONLY (never logged, never an action payload). The channel is **local-only** (Home is desktop-only, PLAN.md 2.9): it is not in `REMOTE_FORWARDED_CHANNELS`, `handleMessage` has no case for it, and the `ws-bridge` stub returns `[]`. The channel constant `TODO_LIST_CHANNEL` lives in `src/shared/capture.ts`. The three coaching flags it feeds (`settings:getStallInterrupt` / `getCommitmentMirror` / `getMorningRitual`) are toggled from `SettingsDialog`, all default OFF.
...
M19 adds the off-app batched nudge preference as the `settings:getOffAppNudge` / `settings:setOffAppNudge` pair: a local-only settings flag, default OFF, mirroring `notifyOnIdle`. Like the other coaching flags it is not in `REMOTE_FORWARDED_CHANNELS`, not a broadcast, and stubbed in `ws-bridge`. The nudge itself never fires on the flag alone: `shouldSendOffAppNudge` (in `src/shared/off-app-nudge.ts`) requires BOTH the flag and a separately confirmed schedule. M19 also wires the R-14 tab-namer gate: when the free-text query opt-in (`FREE_TEXT_QUERY_ENABLED` in `src/shared/free-text-query.ts`, shipped `false`) is ever enabled, `generateTabName` suppresses auto-naming for dashboard-injected tabs (those the `QueryInjector` has armed) so injected free text never reaches the Haiku tab-namer unscrubbed. No new IPC channel is involved in the gate; it is an in-MAIN wiring between `QueryInjector.isDashboardInjected` and the tab-namer.
